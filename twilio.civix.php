...
function _twilio_civix_civicrm_xmlMenu(&$files) {
  foreach (_twilio_civix_glob(__DIR__ . '/xml/Menu/*.xml') as $file) {
    $files[] = $file;
  }
}
 ...
 */
function _twilio_civix_civicrm_postInstall() {
  _twilio_civix_civicrm_config();
  if ($upgrader = _twilio_civix_upgrader()) {
...
function _twilio_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_twilio_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
 ...
 */
function _twilio_civix_civicrm_caseTypes(&$caseTypes) {
  if (!is_dir(__DIR__ . '/xml/case')) {
    return;
  }

  foreach (_twilio_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
 ...
 */
function _twilio_civix_civicrm_angularModules(&$angularModules) {
  if (!is_dir(__DIR__ . '/ang')) {
    return;
  }

  $files = _twilio_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 */
function _twilio_civix_glob($pattern) {
  $result = glob($pattern);
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _twilio_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
 ...
 */
function _twilio_civix_civicrm_alterSettingsFolders(&$metaDataFolders = NULL) {
  static $configured = FALSE;
  if ($configured) {
    return;
  }
...
function _twilio_civix_civicrm_entityTypes(&$entityTypes) {
